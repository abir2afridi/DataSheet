-- SmartSheets Complete Database Setup
-- Paste this entire file into Supabase Dashboard -> SQL Editor -> New query
-- and click "Run" to set up all tables, indexes, triggers, RLS, and buckets.

-- 1. Core Schema (tables, indexes, triggers, RLS, storage)
BEGIN;
...
CREATE TRIGGER on_cell_update AFTER UPDATE ON cells FOR EACH ROW EXECUTE FUNCTION log_cell_version();
...
CREATE POLICY user_workspaces ON workspaces FOR ALL USING (auth.uid() = user_id);
CREATE POLICY user_sheets ON sheets FOR ALL USING (workspace_id IN (SELECT id FROM workspaces WHERE user_id = auth.uid()));
CREATE POLICY user_cells ON cells FOR ALL USING (sheet_id IN (SELECT s.id FROM sheets s JOIN workspaces w ON w.id = s.workspace_id WHERE w.user_id = auth.uid()));
CREATE POLICY user_cell_versions ON cell_versions FOR ALL USING (cell_id IN (SELECT c.id FROM cells c JOIN sheets s ON s.id = c.sheet_id JOIN workspaces w ON w.id = s.workspace_id WHERE w.user_id = auth.uid()));
CREATE POLICY user_cell_comments ON cell_comments FOR ALL USING (cell_id IN (SELECT c.id FROM cells c JOIN sheets s ON s.id = c.sheet_id JOIN workspaces w ON w.id = s.workspace_id WHERE w.user_id = auth.uid()));
CREATE POLICY user_document_blocks ON document_blocks FOR ALL USING (workspace_id IN (SELECT id FROM workspaces WHERE user_id = auth.uid()));
CREATE POLICY user_hybrid_blocks ON hybrid_blocks FOR ALL USING (workspace_id IN (SELECT id FROM workspaces WHERE user_id = auth.uid()));
CREATE POLICY user_hybrid_checklist ON hybrid_checklist_items FOR ALL USING (hybrid_block_id IN (SELECT hb.id FROM hybrid_blocks hb JOIN workspaces w ON w.id = hb.workspace_id WHERE w.user_id = auth.uid()));
CREATE POLICY user_hybrid_cells ON hybrid_cells FOR ALL USING (hybrid_block_id IN (SELECT hb.id FROM hybrid_blocks hb JOIN workspaces w ON w.id = hb.workspace_id WHERE w.user_id = auth.uid()));
CREATE POLICY user_activity_logs ON activity_logs FOR ALL USING (auth.uid() = user_id);
CREATE POLICY user_copy_history ON cell_copy_history FOR ALL USING (auth.uid() = user_id);
CREATE POLICY user_recycle_bin ON recycle_bin FOR ALL USING (auth.uid() = user_id);
CREATE POLICY user_tags ON tags FOR ALL USING (auth.uid() = user_id);
CREATE POLICY user_workspace_tags ON workspace_tags FOR ALL USING (tag_id IN (SELECT id FROM tags WHERE user_id = auth.uid()));
CREATE POLICY user_settings ON user_settings FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY user_avatars ON storage.objects FOR ALL USING (auth.uid()::text = (storage.foldername(name))[1]);
CREATE POLICY user_documents ON storage.objects FOR ALL USING (auth.uid()::text = (storage.foldername(name))[1]);
CREATE POLICY user_exports ON storage.objects FOR ALL USING (auth.uid()::text = (storage.foldername(name))[1]);
